...
{
	?><div class="gdrsserror"><?=\Bitrix\Main\Localization\Loc::getMessage("GD_RSS_READER_NEW_RSS")?></div><?
	...
	foreach($rss->items as $item)
	{
		...
		<div>
			...
			<div class="gdrssitemdetail" id="z<?=$cnt.md5($item["TITLE"])?><?=htmlspecialcharsbx(CUtil::JSEscape($rnd))?>" style="display:none">
				...
				<div class="gdrssitemdesc"><?=$item["DESCRIPTION"]?> <?if($arGadgetParams["SHOW_URL"]=="Y" && preg_match("'^(http://|https://|ftp://)'i", $item["LINK"])):?><a href="<?=htmlspecialcharsbx($item["LINK"])?>"><?echo \Bitrix\Main\Localization\Loc::getMessage("GD_RSS_READER_RSS_MORE")?></a><?endif?></div>
			</div>
		</div><?
	}
	?></div><?
...
{
	?><div class="gdrsserror"><?=\Bitrix\Main\Localization\Loc::getMessage("GD_RSS_READER_RSS_ERROR")?></div><?
}
