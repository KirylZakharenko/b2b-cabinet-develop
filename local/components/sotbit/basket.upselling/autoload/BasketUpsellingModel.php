...
class BasketUpsellingModel extends Query
{
    ...
    */
    ...
    protected function applyPrivatePrices(array $preparedResult): array
    {
        ...
        $productRatio = array_column($preparedResult, 'ratio', ProductTable::getTableName().'ID');
        $privatePrice = (new PrivatePriceAdapter($productId, $productRatio))->get();

        foreach ($preparedResult as $key => $i) {
            $price = $privatePrice[$i[ProductTable::getTableName().'ID']] ? $privatePrice[$i[ProductTable::getTableName().'ID']]  : $preparedResult[$key]['DISPLAY_PRICE'];
        ...
        $this->unpreparedResult = array_map(
            function($i) use($priceId) {
                ...
                $i['DISPLAY_PRICE'] = \CCurrencyLang::CurrencyFormat(
                    ($discountsPrice ?: $i['optimal_price']) * $i['ratio'],
                    $i[PriceTable::getTableName().'CURRENCY'],
                );
