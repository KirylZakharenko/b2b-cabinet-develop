                ...
                while($ob = $res->GetNext()) {
                    if($productsList[$ob["ID"]]){
                        ...
                        elseif ($ob["DETAIL_PICTURE"]){
                            $imgId = $ob["DETAIL_PICTURE"];
                        } else {
                            $resultProduct = CCatalogSku::GetProductInfo($ob["ID"]);
                            if (is_array($resultProduct)) {
                                $infoProduct = CIBlockElement::GetList([], ['ID' => $resultProduct['ID']], false, false, ['PREVIEW_PICTURE', 'DETAIL_PICTURE'])->Fetch();

                                if($infoProduct["PREVIEW_PICTURE"]){
                                    $imgId = $infoProduct["PREVIEW_PICTURE"];
                                }
                                elseif ($infoProduct["DETAIL_PICTURE"]){
                                    $imgId = $infoProduct["DETAIL_PICTURE"];
                                }
                            }
                        }
