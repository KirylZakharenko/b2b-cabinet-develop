...
if(Loader::includeModule("sotbit.auth")) {
    $arResult["EXTENDED_VERSION_COMPANIES"] = Option::get("sotbit.auth", "EXTENDED_VERSION_COMPANIES", "N");
}
...
if ($arResult['ADD_COMPLAINTS_PROPERTIES']) {
    ...
    foreach ($arResult['ADD_COMPLAINTS_PROPERTIES'] as $props) {
        ...
        if ($props["CODE"] === 'ORDER_ID' && $arResult["EXTENDED_VERSION_COMPANIES"] === 'Y') {
             
            $item["HELPER_TEXT"] = Loc::getMessage('SOTBIT_COMPLAINTS_HINT_ORDER_ID');
        }
...
$arResult["COLUMN_MODEL"][0] = '${product.ORIGINAL_NAME}';
...
if ($arResult["ADD_POSITIONS_PROPERTIES"]) {
    foreach ($arResult["ADD_POSITIONS_PROPERTIES"] as $props) {
        ...
        if ($props["PROPERTY_TYPE"] == "N") {
            ...
            $select = '';
            $select = '<select class="form-control" '.$inputName.' data-item="position" data-minimum-results-for-search="Infinity"  data-dropdown-auto-width="true">';
...
$arResult["COLUMN_MODEL"][] = '<a role="button" class="delete-position" data-name="${product.ORIGINAL_NAME}">'.Loc::getMessage('SOTBIT_COMPLAINTS_BTN_REMOVE').'</a>
