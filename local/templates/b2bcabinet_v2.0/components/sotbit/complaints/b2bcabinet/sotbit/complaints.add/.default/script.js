const DatatableAdvanced = function () {
    ...
    const _componentDatatableAdvanced = function () {
        ...
        $.extend($.fn.dataTable.defaults, {
            ...
            order: [[0, 'asc']],
            dom: '<"datatable-header"f<"datatable-add">l><"datatable-content"<"datatable-scroll"t><"datatable-footer"ip>>',
            language: {
        ...
        $('.datatable-dom-position').DataTable({
            dom: `<"datatable-header length-left"lp>
                  <"datatable-add">
                  <"datatable-scroll"t>
                  <"datatable-footer info-right"fi>`,
        });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const nodeOrderId = document.querySelector('[name="COMPLAINTS[PROPERTIES][ORDER_ID]"]');
    let currentOrderId;

    if (nodeOrderId) {
        currentOrderId = nodeOrderId.value
    }
            ...
            if (data.data.product) {
                if (!BX.util.in_array(data.data.product.ID, DatatableAdvanced.listProducts)) {
                    ...
                    BX.onCustomEvent('B2BNotification', [
                        BX.message('SOTBIT_POSITION_NAME_ADD_NOTIFICATION') + data.data.product.ORIGINAL_NAME + "<br>" +
    ...
    function addProduct() {
        if (comlaintsType == "ORDER") {
            if (nodeOrderId?.value !== "") {
                if (currentOrderId !== nodeOrderId?.value) {
                    SweetAlert.confirm(BX.message("SOTBIT_CLEAR_PRODUCTS_IN_COMPLAINTS"), () => {
                        currentOrderId = nodeOrderId.value;
                        clearTableComplaints();
                        setEmptyRow();
                        openSidePanel(searchPage + '?orderId=' + nodeOrderId.value);
                    });
                } else {
                    openSidePanel(searchPage + '?orderId=' + nodeOrderId.value);
                }
    ...
    function clearTableComplaints() {
        $tablePosition.DataTable().clear().draw();
    }
});