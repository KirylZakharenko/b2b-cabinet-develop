                    ...
                    <form name="addOrg" method="post" id="add-org"
                                    ...
                                    <div class="mt-2">
                                        ...
                                        foreach ($arResult['PERSON_TYPES'] as $id => $name) : ?>
                                            ...
                                            if ($id == $arResult['PERSON_TYPE']['ID']) {
                                                $currentPersonType = $id;
                                            } elseif ($isFirstPersonType) {
                                                $currentPersonType = $id;
                                            }

                                            $isFirstPersonType = false;
                                            ?>
                        ...
                        <div class="row d-flow-root mt-4">
                            ...
                            if ($arResult["USE_PERSONAL_GROUPS"] && $arResult["PERSONAL_ACTIVE_GROUP"] == $groupId && in_array($arResult["PERSON_TYPE"]["ID"],
                                ...
                                $personTypeID = $currentPersonType;
                            } else {
                                $personTypeID = $arResult["PERSON_TYPE"]['ID'];
                            }
                                                            ...
                                                            if ($property["TYPE"] == "CHECKBOX" || $property["TYPE"] == 'Y/N') {
                                                                ...
                                                                >
                                                                    <? if (!$property["DEFAULT_VALUE"]): ?>
                                                                        <option disabled selected><?= Loc::getMessage("SOA_FIELD_TYPE_ENUM") ?></option>
                                                                    <? endif; ?>
                                                                ...
                                                                <div class="sale-personal-profile-detail-property-file">
                                                                    <?
                                                                    if (count($profileFiles) > 0 && !empty($currentValue))
                                                                    {
                                                                        ?>
                                                                        ...
                                                                        foreach ($profileFiles as $file)
                                                                        {
                                                                            ...
                                                                            if ($arFile) {
                                                                                ?>
                                                                                <div class="sale-personal-profile-detail-form-file d-flex align-items-center gap-3 mb-2">
                                                                                    <?
                                                                                    if (CFile::IsImage($arFile['FILE_NAME']))
                                                                                    {
                                                                                        ?>
                                                                                        <div class="sale-personal-profile-detail-prop-img rounded-2 overflow-hidden">
                                                                                            <?=CFile::ShowImage($arFile['ID'], 78, 78, "border=0", "", true)?>
                                                                                        </div>
                                                                                        <?
                                                                                    }
                                                                                    else
                                                                                    {
                                                                                        ?>
                                                                                        <a class="text-break" download="<?=$arFile["ORIGINAL_NAME"]?>" href="<?=CFile::GetFileSRC($arFile)?>">
                                                                                            <?=Loc::getMessage('SPPD_DOWNLOAD_FILE', array("#FILE_NAME#" => $arFile["ORIGINAL_NAME"]))?>
                                                                                        </a>
                                                                                        <?
                                                                                    }
                                                                                    ?>
                                                                                    <div class="form-check text-nowrap">
                                                                                        <input type="checkbox" value="<?=$arFile['ID']?>" class="form-check-input profile-property-check-file" id="profile-property-check-file-<?=$arFile['ID']?>">
                                                                                        <label class="form-check-label" for="profile-property-check-file-<?=$arFile['ID']?>"><?=Loc::getMessage('SOA_SALE_DELETE_FILE')?></label>
                                                                                    </div>
                                                                                </div>
                                                                                <?
                                                                            }
