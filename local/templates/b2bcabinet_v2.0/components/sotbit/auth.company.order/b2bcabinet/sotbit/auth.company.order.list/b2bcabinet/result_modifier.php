...
$obRes = \Bitrix\Sale\Internals\OrderPropsValueTable::getList([
    'select' => [
        'ORDER_ID', 
        'ORDER_PROPS_ID', 
        'ORDER_VALUE' => 'VALUE', 
        'COMPANY_VALUE' => 'COMPANY_PROP.VALUE',
        'COMPANY_ID' => 'COMPANY_PROP.COMPANY_ID',
        'CODE'
    ],
    'filter' => [
        'ORDER_ID' => array_keys($arResult['ORDERS']),
        'CODE' => array_merge($innProps, $orgProps),
        '!VALUE' => ''
    ],
    'group' => [
        'ORDER_ID'
    ...
    'runtime' => [
        new \Bitrix\Main\Entity\ReferenceField(
            'COMPANY_PROP',
            \Sotbit\Auth\Internals\CompanyPropsValueTable::class,
            [
                'this.ORDER_PROPS_ID' => 'ref.PROPERTY_ID', 
...
while($prop = $obRes->fetch())
{
    if (empty($arOrganizations[$prop['ORDER_ID']]['ID'])) {
        $arOrganizations[$prop['ORDER_ID']]['ID'] = $prop['COMPANY_ID'];
    }
    if(in_array($prop['CODE'],$orgProps))
    {
        $arOrganizations[$prop['ORDER_ID']]['ORG']  = $prop['COMPANY_VALUE'] ?: $prop['ORDER_VALUE'];
    }
    if(in_array($prop['CODE'],$innProps))
    {
        $arOrganizations[$prop['ORDER_ID']]['INN'] = $prop['COMPANY_VALUE'] ?: $prop['ORDER_VALUE'];
    }
}
...
foreach($arResult['ORDERS'] as $arOrder)
{
    ...
    $arResult['ROWS'][] = array(
        'data' =>array_merge($arOrder['ORDER'], array(
            ...
            'BUYER' => $arOrganizations[$arOrder['ORDER']['ID']]['ID'] ?
                        '<a href="'. $methodIstall .'personal/companies/profile_detail.php?ID='. $arOrganizations[$arOrder['ORDER']['ID']]['ID'] .'">'. $arOrganizations[$arOrder['ORDER']['ID']]['ORG'] . ($arOrganizations[$arOrder['ORDER']['ID']]['INN'] ? ' (' . $arOrganizations[$arOrder['ORDER']['ID']]['INN'] . ')' : '') .'</a>':
                        '<span>' . $arOrganizations[$arOrder['ORDER']['ID']]['ORG'] . ($arOrganizations[$arOrder['ORDER']['ID']]['INN'] ? ' (' . $arOrganizations[$arOrder['ORDER']['ID']]['INN'] . ')' : '')
        ) ),
        'actions' => $aActions,
        'editable' => true,
    );
