...
$PROPERTY_OFFER_ARTICLE = 'PROPERTY_' . ($arParams['ARTICLE_OFFER_PROPERTY_CODE'] ?: 'ARTNUMBER') . '_VALUE';

/* init arrays for bitrix.grid */
...
foreach ($arResult['ORDER_PROPS'] as $key => $ORDER_PROP) {
    if (!empty($ORDER_PROP['VALUE'])) {
        if (!($ORDER_PROP['MULTIPLE'] == "Y" && empty(array_diff(is_array(unserialize($ORDER_PROP['VALUE'])) ? unserialize($ORDER_PROP['VALUE']) : [], [""]))))
...
if (!empty($arResult['BASKET'])) {
    ...
    foreach ($arResult['BASKET'] as $key => &$item) {
        ...
        $productCols = [
            'ID' => $item['ID'],
            'ARTICLE' => $item[$PROPERTY_OFFER_ARTICLE] ?: $item[$PROPERTY_ARTICLE],
            'NAME' => $item['NAME'],
    ...
    foreach ($arResult['BASKET'] as $key => &$item) {
        ...
        $arResult['PRODUCT_2_ROWS'][] = [
            'data' => [
                'ID' => $item['ID'],
                'ARTICLE' => $item[$PROPERTY_OFFER_ARTICLE] ?: $item[$PROPERTY_ARTICLE],
                'NAME' => '<a ' . ($item['DETAIL_PAGE_URL'] ? 'href="' . $item['DETAIL_PAGE_URL'] . '" target="_blank"' : '') . '>' . $item['NAME'] . '</a>',
                'QUANTITY' => $item['QUANTITY'],
                'SUM' => $item['FORMATED_SUM']
            ],
            'actions' => [],
...
if (Loader::includeModule('sotbit.complaints') &&
    ...
    $complaintsPositionsIblock = Option::get('sotbit.complaints', 'IBLOCK_COMPLAINTS_POSITIONS_ID', '', SITE_ID);
    $propertyBindPositions = Option::get('sotbit.complaints', 'PROPERTY_BINDING_POSITIONS', '', SITE_ID);
    $propertyBindProduct = Option::get('sotbit.complaints', 'PROPERTY_BINDING_PRODUCTS', '', SITE_ID);
    ...
    $obComplaints = CIBlockElement::GetList(array(), $arFilter, false, array(), $arSelect);

    while ($complaint = $obComplaints->fetch()) {
        $positionsId = [];
        $complaintPositions = [];
        $summProducts = null;

        $obComp = CIBlockElement::GetProperty(
            $complaintsIblock,
            $complaint['ID'],
            [],
            [
                "CODE" => $propertyBindPositions
            ]
        );

        while ($propComp = $obComp->fetch()) {
            if ($propComp["VALUE"]) {
                $positionsId[] = $propComp["VALUE"];
            }
        }

        $filterPositions = ["ID" => $positionsId, "IBLOCK_ID" => $propertyComplaintsPositionsId];
        ...
        while ($arElement = $rsElement->GetNextElement()) {
            $propertyProductId = $arElement->GetProperties()[$propertyBindProduct];
            $productsId = array_column($arResult['BASKET'], 'PRODUCT_ID', 'ID');
            $key = array_search($propertyProductId["VALUE"], $productsId);
            $sumProducts += $arResult['BASKET'][$key]['SUM'];
        }
        ...
        $arResult['COMPLAINTS_ROW'][] = [
            'data' => [
                'ID' => $complaint['ID'],
                'NAME' => $complaint['NAME'],
                'STATUS' => $complaint['PROPERTY_COMPLAINT_STATUS_VALUE'],
                'SUM' => CCurrencyLang::CurrencyFormat($sumProducts, $arResult["CURRENCY"], true),
                'COMPANY' => $companyName,
            ],
            'actions' => [
                [
                    'TEXT' => Loc::getMessage('COMPLAINTS_OPEN'),
                    'ONCLICK' => "document.location.href='" . $complaintsPath . "detail/" . $complaint['ID'] . "/'",
                ],
            ]
        ];
    }
