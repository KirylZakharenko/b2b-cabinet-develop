    ...
    SCGadget.prototype.ShowSettings = function(id, t)
    {
        ...
        BX.ajax({
            ...
            onsuccess: function(data)
            {
                    ...
                    if (gdObject.hasOwnProperty(param_id))
                    {
                        ...
                        if(param["TYPE"]=="STRING")
                        {
                            str = '<span class="form-label">'+param["NAME"] + ':</span><input type="text" class="form-control form-control-sm" name="GP_' + param_id + '" id="' + input_id + '" size="40"' +
                                ' value="'+(param["VALUE"] ? jsUtils.htmlspecialchars(param["VALUE"]) : '')+'">';
                        }
                ...
                oEl.innerHTML = '<input type="button" class="btn btn-sm btn-flat-primary" value="'+langGDButtonConfirm+'" onclick="getGadgetHolderSC(\''+_this.gadgetHolderID+'\').UpdSettings(\''+id+'\');"> <input type="button" class="btn btn-sm btn-flat-danger" value="'+langGDButtonCancel+'" onclick="getGadgetHolderSC(\''+_this.gadgetHolderID+'\').CloseSettingsForm(\''+id+'\');">';
                dSet.appendChild(oEl);
