...
<div class="sotbit_cabinet">
	...
	?>
		<script type="text/javascript">
			...
			var langGDError1 = '<?= CUtil::JSEscape(Loc::getMessage("CMDESKTOP_TDEF_ERR1")) ?>';
			var langGDError2 = '<?= CUtil::JSEscape(Loc::getMessage("CMDESKTOP_TDEF_ERR2")) ?>';
			var langGDConfirm1 = '<?= CUtil::JSEscape(Loc::getMessage("CMDESKTOP_TDEF_CONF")) ?>';
			var langGDConfirmUser = '<?= CUtil::JSEscape(Loc::getMessage("CMDESKTOP_TDEF_CONF_USER")) ?>';
			var langGDConfirmGroup = '<?= CUtil::JSEscape(Loc::getMessage("CMDESKTOP_TDEF_CONF_GROUP")) ?>';
			var langGDClearConfirm = '<?= CUtil::JSEscape(Loc::getMessage("CMDESKTOP_TDEF_CLEAR_CONF")) ?>';
			var langGDClearTitleConfirm = '<?= CUtil::JSEscape(Loc::getMessage("CMDESKTOP_TDEF_TITLE_CLEAR_CONF")) ?>';
			var langGDButtonCancel = "<? echo CUtil::JSEscape(Loc::getMessage("CMDESKTOP_TDEF_CANCEL")) ?>";
			var langGDButtonConfirm = "<? echo CUtil::JSEscape(Loc::getMessage("CMDESKTOP_TDEF_CONFIRM")) ?>";
		</script>
	<?
	}
		...
		<div class="sw--all_widgets">
			<div class="widget-bx-gd-buttons gap-4 position-fixed">
				<div class="fab-menu fab-menu-bottom fab-menu-bottom-end" data-fab-position="custom" data-fab-toggle="click">
					<button type="button"
							aria-label="<?= GetMessage('PERSONAL_MANAGER_LABEL') ?>"
							class="fab-menu-btn btn btn-primary btn-icon widget-btn">
						<div class="m-1">
								...
								<div class="card-header d-flex position-relative" <? if ($GLOBALS["USER"]->IsAuthorized()) : ?> style="cursor:move;" onmousedown="return getGadgetHolderSC('<?= AddSlashes($arResult["ID"]) ?>').DragStart('<?= $arGadget["ID"] ?>', event)" <? endif; ?>>
									...
									<div class="d-inline-flex ms-auto align-items-center">
										...
										?>
											...
											if ($bChangable) {
											?>
												<a class="text-body px-1 gdsettings <?= ($arGadget["NOPARAMS"] ? ' gdnoparams' : '') ?>" href="javascript:void(0)" onclick="return getGadgetHolderSC('<?= AddSlashes($arResult["ID"]) ?>').ShowSettings('<?= $arGadget["ID"] ?>');" title="<?= Loc::getMessage("CMDESKTOP_TDEF_SETTINGS") ?>"">
														<i class="ph ph-pencil-simple" onmousedown="event.stopPropagation();"></i>
												</a>
												<a class="text-body px-1" data-card-action="remove" href="javascript:void(0)" onclick="return getGadgetHolderSC('<?= AddSlashes($arResult["ID"]) ?>').Delete('<?= $arGadget["ID"] ?>');" title="<?= Loc::getMessage("CMDESKTOP_TDEF_DELETE") ?>">
										...
										?>
										<a class="text-body px-1" href="javascript:void(0)" onclick="savePositionCollapse('<?= $arResult["ID"] ?>');" title="<?= Loc::getMessage("CMDESKTOP_TDEF_HIDE") ?>" data-card-action="collapse">
