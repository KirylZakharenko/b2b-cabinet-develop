function SupQuoteMessage(id)
{
	...
	else
	{
		...
		const textData = (el.innerText) ? el.innerText : el.textContent;
		const cardMessage = el.closest('.card');
		if(el)
		{
			let str = textData;
			str = str.replace(/\r\n\r\n/gi, "_newstringhere_");
			...
			str = `<quote>${str}</quote><p><br></p>`;

			if (cardMessage) {
				str = `<strong class="text-dark">${cardMessage.querySelector('.card-header').innerText}</strong>${str}`;
			}

			$('#MESSAGE').trumbowyg('html', `${$('#MESSAGE').trumbowyg('html') + str}`);
		}
	}
}
