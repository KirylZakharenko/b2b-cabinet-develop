	...
	(function () {
	  ...
	  BX.Grid.ActionPanel.prototype = {
	    init: function init(parent, actions, types) {
		  ...
		  BX.bind(BX(this.parent.getId()+'-view'), 'click', BX.delegate(this._clickOnView, this));
		},
		...
		_clickOnView: function _clickOnView(event) {
			let button = null;

			if (button = event.target.closest('[data-type-view]')) {
				[].forEach.call(event.target.closest('.main-grid-view__wrapper').children, item => item.classList.remove('active'));
				button.classList.add('active');

				BX.ajax.runAction('sotbit:b2bcabinet.viewuigrid.add', {
					data: {
						gridId: 'organizations_' + this.parent.containerId,
						typeView: button.dataset.typeView,
					}
				})

				if (button.dataset.typeView === 'list') {
					this.parent.container.classList.remove('main-grid-block');
					this.parent.container.classList.add('main-grid-list');
				} else {
					this.parent.container.classList.remove('main-grid-list');
					this.parent.container.classList.add('main-grid-block');
				}
			}
		},
