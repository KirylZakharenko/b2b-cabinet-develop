...
use Sotbit\B2bcabinet\Controller\ViewUIGrid;
...
if ($arParams["SHOW_ROW_CHECKBOXES"])
{
	$gridClasses[] = 'main-grid-checkboxes';
}
...
$arrTypeView = [
	[
		'iconClass' => 'ph-rows', 
		'type' => 'list',
		'title' => Loc::getMessage('interface_grid_view_type_list')],
	[
		'iconClass' => 'ph-squares-four', 
		'type' => 'block',
		'title' => Loc::getMessage('interface_grid_view_type_block')
	],
];

$gridView = new ViewUIGrid(Application::getInstance()->getContext()->getRequest());
$activeTypeView = $gridView->getActiveView('organizations_' . $arParams['GRID_ID'], 'list');

$gridClasses[] = 'main-grid-' . $activeTypeView;
?>
<? if ($arParams["HIDDEN_TYPE_GRID"] !== "Y"): ?>
<div id="<?=$arParams["GRID_ID"]?>-view" class="main-grid-view">
	<div class="main-grid-view__wrapper">
		<? foreach($arrTypeView as $item): ?>
			<button 
				class="main-grid-view__button <?= $activeTypeView === $item['type'] ?' active' : '' ?>"
				data-type-view="<?= $item['type'] ?>"
				type="button"
				title="<?=$item['title']?>">
...
<div class="<?=join(' ', $gridClasses)?>" id="<?=$arParams["GRID_ID"]?>" data-ajaxid="<?=$arParams["AJAX_ID"]?>"<?=$arResult['IS_AJAX'] ? " style=\"display: none;\"" : ""?>><?
					...
					?><table class="main-grid-table" id="<?=$arParams["GRID_ID"]?>_table">
						<thead class="main-grid-header" data-relative="<?=$arParams["GRID_ID"]?>"><?
							...
							if (!empty($arResult['ROWS']) || (count($arParams['ROWS']) === 1 && $arParams['ROWS'][0]['id'] === 'template_0')) :
								...
								?><tr class="main-grid-row main-grid-row-body<?=$arRow["layout"]["row"]["class"]?> <?=!empty($arRow["actions"]) ? 'main-grid-row-body-actions' : ''?>"<?=$arRow["layout"]["row"]["attributes"]?>><?
									if ($arRow["layout"]["columns"]["drag"]["cell"]["enabled"]) :
										...
										foreach ($arResult['COLUMNS'] as $id => $header):
											...
											?><td class="<?=$colLayout["cell"]["class"]?>"<?=$colLayout["cell"]["attributes"]?> data-code="<?= $id ?>"><?
												?><div class="main-grid-cell-inner"><?
													...
													<span class="main-grid-cell-name text-nowrap">
														<?= $arResult['COLUMNS'][$id]['name']; ?>
													</span>
													<span class="main-grid-cell-content"<?=$colLayout["container"]["attributes"]?>><?
														if ($colLayout["plusButton"]["enabled"]) :
															?><span class="main-grid-plus-button"></span><?
												...
												?></div><?
												if ($arRow["layout"]["columns"]["actions"]["button"]["enabled"] && $header['sort_index'] === 0):
													if (empty($arParams['VIEW_ACTIONS'])): ?>
													<div class="main-grid-cell-actions">
														<? foreach ($arRow["actions"] as $action) : ?>
															<a class="main-grid-cell-action me-2 <?=$action['HIDDEN'] === 'Y' ? 'main-grid-cell-action-hidden' : ''?>" href="#" onclick="<?= $action['onclick'] ?>">
																<? if (!empty($action['ICON'])) : ?>
																	<i class="<?=$action['HIDETEXT'] != 'Y' ? 'me-2' : ''?> <?=$action['ICON']?>"></i>
																<? endif; ?>
																<?= $action['HIDETEXT'] != 'Y' ? $action['text'] : '' ?> 
															</a>
														<? endforeach; ?>
													</div><?
													elseif(!empty($arRow["actions"]["eye"])): ?>
													<div class="main-grid-cell-actions">
														<a class="main-grid-cell-action me-2 <?=$arRow["actions"]["eye"]['HIDDEN'] === 'Y' ? 'main-grid-cell-action-hidden' : ''?>" href="#" onclick="<?= $arRow["actions"]["eye"]['onclick'] ?>">
															<? if (!empty($arRow["actions"]["eye"]['ICON'])) : ?>
																<i class="<?=$arRow["actions"]["eye"]['HIDETEXT'] != 'Y' ? 'me-2' : ''?> <?=$arRow["actions"]["eye"]['ICON']?>"></i>
															<? endif; ?>
															<?= $arRow["actions"]["eye"]['HIDETEXT'] != 'Y' ? $arRow["actions"]["eye"]['text'] : '' ?> 
														</a>
													</div><?
													endif;
												endif;
										...
										if ($arRow["layout"]["columns"]["checkbox"]["cell"]["enabled"]): ?><?
											?><td class="main-grid-cell main-grid-cell-checkbox p-0"><?
												?><div class="d-none"><?
										...
										if (!empty($arRow["columns"]["LAMP"])): 
											?><td class="main-grid-cell main-grid-cell-lamp">
												<?=$arRow["columns"]["LAMP"]?>
											</td><?
											endif;?><?

										if ($arRow["layout"]["columns"]["actions"]["button"]["enabled"]){
											switch($arParams['VIEW_ACTIONS']) { 
												case "table": ?>
													<td class="main-grid-cell main-grid-cell-actions">
														<div class="main-grid-cell-actions-wrapper">
														<? foreach ($arRow["actions"] as $action) : ?>
															<? if ($key == 'eye') continue; ?>
															<a class="main-grid-cell-action <?=$action['CLASS'] ? $action['CLASS'] : '' ?>" href="#" onclick="<?= $action['onclick'] ?>">
																<?=$action['text']?>
															</a>
														<? endforeach; ?>
														</div>
													...
													break;
												case "table-dropdown": ?>
													<td class="main-grid-cell main-grid-cell-actions main-grid-cell-actions-dropdown">
														<div class="main-grid-cell-actions-wrapper">
															<button id="dropdown<?=$arParams['GRID_ID'] . '_' . $key?>" type="button" class="btn p-2 border-0" data-bs-toggle="dropdown">
																<i class="ph-dots-three-vertical fw-bold"></i>
															</button>
															<div class="dropdown-menu" aria-labelledby="dropdown<?=$arParams['GRID_ID'] . '_' . $key?>">
															<? foreach ($arRow["actions"] as $key => $action) : ?>
																<? if ($key == 'eye') continue; ?>
																<a class="dropdown-item main-grid-cell-action <?=$action['CLASS'] ? $action['CLASS'] : '' ?>" href="#" onclick="<?= $action['onclick'] ?>">
																	<? if (!empty($action['ICON'])) : ?>
	...
	?></div><?
?></div><?
	?><div class="main-grid-bottom-panels card card-position-sticky card-bottom--1
		<?=($arParams["SHOW_NAVIGATION_PANEL"] && !empty($arResult["NAV_STRING"])) || 
		   $arParams["SHOW_ACTION_PANEL"] && isset($arParams["ACTION_PANEL"]) && !empty($arParams["ACTION_PANEL"]) && is_array($arParams["ACTION_PANEL"]["GROUPS"]) ?
		   '' : 'd-none'?>" id="<?=$arParams["GRID_ID"]?>_bottom_panels"><?
		?><div class="main-grid-nav-panel"><?
			?><div class="main-grid-more" id="<?=$arParams["GRID_ID"]?>_nav_more"><?
				...
				?></a><?
			?></div><?
		if ($arParams["SHOW_NAVIGATION_PANEL"] && !empty($arResult["NAV_STRING"])) : ?><?
			?><div class="main-grid-panel-wrap"><?
				?><table class="main-grid-panel-table main-grid-panel-table-section"><?
					?><tr><?
						if (!empty($arResult["NAV_STRING"]) && (isset($arResult["TOTAL_ROWS_COUNT"]) && $arParams["SHOW_PAGINATION"] || !empty($arParams["TOTAL_ROWS_COUNT_HTML"]))) : ?><?
						...
						if ($arParams["SHOW_PAGINATION"]) : ?><?
							?><td class="main-grid-panel-cell main-grid-panel-cell-pagination main-grid-cell-left float-end text-capitalize"><?
		...
		<? if ($arParams["SHOW_ACTION_PANEL"] && isset($arParams["ACTION_PANEL"]) && !empty($arParams["ACTION_PANEL"]) && is_array($arParams["ACTION_PANEL"]["GROUPS"])) : ?><?
			?><div class="main-grid-action-panel main-grid-disable"><?
				?><div class="main-grid-control-panel-wrap card-body"><?
							...
							foreach ($arParams["ACTION_PANEL"]["GROUPS"] as $groupKey => $group) : ?><?
								?><td class="main-grid-control-panel-cell<?= isset($group["CLASS"]) && $group["CLASS"] ? " ".$group["CLASS"] : "" ?>"><?
									foreach ($group["ITEMS"] as $itemKey => $item) : ?><?
										...
										if ($item["TYPE"] === "BUTTON") : ?><?
											?><span class="btn main-grid-panel-control-container main-grid-buttons <?=Text\HtmlFilter::encode($item["CLASS"])?>" data-name="<?=Text\HtmlFilter::encode($item["NAME"] ?? '')?>" data-value="<?=Text\HtmlFilter::encode($item["VALUE"] ?? '')?>" id="<?=Text\HtmlFilter::encode($item["ID"])?>_control" data-onchange="<?=Text\HtmlFilter::encode(CUtil::PhpToJSObject($item["ONCHANGE"]))?>" title="<?=Text\HtmlFilter::encode($item["TITLE"] ?? '')?>"><?
												?><?=$item["TEXT"]
											?></span><?
										endif; ?><?
										if ($item["TYPE"] === "LINK") :
								...
								?><td class="main-grid-control-panel-cell"><?
									if ($arParams["SHOW_CHECK_ALL_CHECKBOXES"]): ?><?
										?><div class="form-check"><?
											?><input class="form-check-input main-grid-check-all" id="<?=$arParams["GRID_ID"]?>_check_all" type="checkbox" title="<?=getMessage('interface_grid_check_all') ?>"<? if (!$arResult['ALLOW_EDIT']): ?> disabled<? endif ?>><?
											?><label class="form-check-label text-muted" for="<?=$arParams["GRID_ID"]?>_check_all">
												<?=Loc::getMessage("interface_grid_settings_select_all_columns")?>
											</label><?
										?></div><?
									endif; ?><?
								?></td><?
