...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters)
		{
			...
			this.templateSignsString = parameters.templateSignsString || '';
			this.siteId = parameters.siteID || '';
			this.ajaxUrl = parameters.ajaxUrl || '';
			this.templateFolder = parameters.templateFolder || '';
			this.siteFolder = parameters.siteFolder || '';
			this.defaultBasketItemLogo = this.templateFolder + "/images/product_logo.png";
			this.defaultStoreLogo = this.siteFolder + "/assets/images/no_photo.svg";
		...
		sendRequest: function(action, actionData)
		{
			...
			else
			{
				...
				data.sotbit_set_site_template = this.templateSignsString;
				$.ajax({
					method: 'POST',
					dataType: 'json',
					...
					success: BX.delegate(function(result) {
						if (result.order && result.order.USER_PROFILES)
		...
		animateScrollTo: function(node, duration, shiftToTop)
		{
			...
			var orderBlockPos = BX.pos(this.orderBlockNode),
			...
			$('.content-inner').animate({ scrollTop: orderBlockPos.height + ghostTop - orderBlockPos.bottom, duration: duration }, 300);
		},
		...
		checkNotifications: function()
		{
			...
			if (informer)
			{
				if (this.firstLoad && this.result.IS_AUTHORIZED && typeof this.result.LAST_ORDER_DATA.FAIL === 'undefined')
				{
						...
						BX.create('DIV', {
							...
							children: [
								BX.create('DIV', {
									...
									children: [
										BX.create('DIV', {props: {className: 'ph-' + (success ? 'check' : 'warning') + '-circle me-2'}}),
										text
									]
								})
							]
					...
					BX.addClass(informer, 'alert alert-' + className + ' alert-dismissible fade show');
			...
			if (pagination.pages.length > 1)
			{
				navigation.push(
					BX.create('LI', {
						...
						props: {className: 'page-item page-item-arrow'},
						html: pagination.pageNumber == 1
							? '<span class="page-link rounded disabled" title="' + this.params.MESS_NAV_BACK + '"><i class="ph-caret-left fw-semibold"></i></span>'
							: '<a class="page-link rounded" href="" title="' + this.params.MESS_NAV_BACK + '"><i class="ph-caret-left fw-semibold"></i></a>',
				...
				for (i = 0; i < pagination.pages.length; i++)
				{
					...
					navigation.push(
						BX.create('LI', {
							...
							props: {className: 'page-item ' + active},
							html: '<a class="page-link rounded" href=""><span>' + pageCounter  + '</span></a>',
							events: {click: BX.proxy(this.doPagination, this)}
						})
					);
				}
				...
				navigation.push(
					BX.create('LI', {
						...
						html: pagination.pageNumber == pagination.pages.length
							? '<span class="page-link rounded disabled" title="' + this.params.MESS_NAV_FORWARD + '"><i class="ph-caret-right fw-semibold"></i></span>'
							: '<a class="page-link rounded" href="" title="' + this.params.MESS_NAV_FORWARD + '"><i class="ph-caret-right fw-semibold"></i></a>',
						events: {click: BX.proxy(this.doPagination, this)}
					})
				);
					...
					children: [
						BX.create('DIV', {
							...
							children: [BX.create('UL', {
										props: {className: 'pagination pagination-flat'},
										children: navigation
									})]
						})
					]
		...
		alignBasketColumns: function()
		{
			...
			if (windowSize.innerWidth > 576 && windowSize.innerWidth < 767)
			{
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			...
			cols = [
				BX.create('DIV', {
					props: {className: 'bx-soa-item-td bx-soa-item-td-first'},
					children: [
						itemImgBlock,
						BX.create('DIV', {
		...
		getCouponsLabel: function(active)
		{
			return BX.create('LABEL', {
				...
				html: active ? this.params.MESS_USE_COUPON + ':' : this.params.MESS_COUPON + ':',
			});
		},
		...
		getDeliveryLocationInput: function(node)
		{
			...
			if (location && location[0] && location[0].output)
			{
				...
				labelHtml = '<label class="form-label" for="soa-property-' + parseInt(locationId) + '">'
			...
			if (profilesLength)
			{
				...
				)
				{
					...
					input = BX.create('SELECT', {
						props: {className: 'form-control select', name: 'PROFILE_ID'},
						dataset: {
					...
					$(input).select2().on('change', BX.delegate(function(){
				...
				else
				{
					for (i in this.result.USER_PROFILES)
					{
						...
						{
							node.classList.add('bx-soa-section-hidden');
							node.appendChild(
								BX.create('INPUT', {
									props: {
										name: 'PROFILE_ID',
		...
		createPaySystemItem: function(item)
		{
			...
			if (item.DESCRIPTION) {
				...
				tooltipPay.setAttribute('data-bs-popup', 'tooltip');
				tooltipPay.setAttribute('data-bs-html', true);
				tooltipPay.setAttribute('data-bs-original-title', item.DESCRIPTION);
				tooltipPay.setAttribute('data-bs-placement', 'right');
			}
				...
				children: [
					BX.create('INPUT', {
						props: {
							...
							name: 'PAY_SYSTEM_ID',
							type: 'checkbox',
							className: 'bx-soa-pp-company-checkbox form-check-input',
							value: paySystemId,
							checked: checked
						}
					}),
			...
			if (item.DESCRIPTION) {
				tooltip = BX.create('DIV', {
					...
					children: [
						BX.create('I', {
							props: {
								className: 'ph-question'
							}
						})
					]
				});
				tooltip.setAttribute('data-bs-popup', 'tooltip');
				tooltip.setAttribute('data-bs-html', true);
				tooltip.setAttribute('data-bs-original-title', item.DESCRIPTION);
		...
		createPickUpItem: function(currentStore, options)
		{
			...
			if (this.params.SHOW_STORES_IMAGES === 'Y')
			{
				...
				logoNode = BX.create('IMG', {
					props: {
						src: imgSrc,
						className: 'bx-soa-pickup-l-item-img rounded-3'
					},
					events: {
						click: BX.delegate(function(e){
							this.popupShow(e, logotype && logotype.src_orig || imgSrc);
			...
			storeNode = BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {
						props: {className: 'bx-soa-pickup-l-item-adress mb-1'},
		...
		editActivePropsBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				propsNode = BX.create('DIV', {props: {className: 'row d-flow-root'}});
				selectedDelivery = this.getSelectedDelivery();
		...
		editPropsItems: function(propsNode)
		{
			...
			while (group = groupIterator())
			{
				...
				let nameGroupNode = BX.create('h6', {
					props: {className: 'bx-soa-section-title-container fw-bold pb-0 mb-3'},
					text: group.getName()
				});

				propsItemsContainer = BX.create('DIV', {
					props: {className: 'col-sm-6 mb-4 bx-soa-customer ' + (counProp%2 === 0 ? 'float-start' : 'float-end')},
		...
		insertStringProperty: function(property, propsItemNode, disabled)
		{
			...
			else
			{
				...
				propsItemNode.appendChild(propContainer);
				// if (settings.IS_READONLY == 'Y' && textNode) {
				// 	textNode.setAttribute('disabled', true);
				// }
				this.alterProperty(settings, propContainer);
				this.bindValidation(property.getId(), propContainer);
			}
		},
		...
		insertEnumProperty: function(property, propsItemNode, disabled)
		{
			...
			else
			{
				...
				if (settings.VALUE?.length === 0) {
					propsItemNode.querySelector('select').value = '';
				}
				this.alterProperty(settings, propContainer);
				this.bindValidation(property.getId(), propContainer);
				this.bindMultiAddress(settings, propContainer);
			}
		},
		...
		insertYNProperty: function(property, propsItemNode, disabled)
		{
			...
			else
			{
				...
				const checkbox = propContainer.querySelector('[type="checkbox"]');
				const settings = property.getSettings();

				checkbox.classList.add('form-check-input');
				checkbox.setAttribute('id', 'soa-property-' + property.getId());
		...
		alterProperty: function(settings, propContainer)
		{
			...
			for (i = 0; i < inputs.length; i++){
				...
				if (settings.IS_READONLY && settings.ID !== "MULTI_ADDRESS") {
			...
			{
				BX.addClass(add[i], 'btn btn-sm');

				if (settings.IS_READONLY == 'Y')
					add[i].setAttribute('disabled', true);
		...
		bindMultiAddress: function(settings, propContainer)
		{
			if (settings.ID !== 'MULTI_ADDRESS')
				return;
			
			const selectAddress = propContainer.querySelector('select');
			if (selectAddress) {
				$(selectAddress).on('change', BX.delegate(function(event){
					this.setAddressOrder(event.target.value, settings, propContainer);
				}, this));
				setTimeout(this.setAddressOrder.bind(this, selectAddress.value, settings, propContainer), 0);
			}
		},

		setAddressOrder: function(address, settings, propContainer)
		{
			const arrAddress = address.split(';').map(item => item.trim());

			if (!arrAddress)
				return;

			settings.ADDRESS_PROP.forEach((item, key) => {
		...
		editPropsComment: function(propsNode)
		{
			...
			propsCommentContainer = BX.create('DIV', {props: {className: 'col-sm-12 mt-3 float-start'}});
			label = BX.create('LABEL', {
		...
		editTotalBlock: function()
		{
			...
			for(i in this.result.GRID.ROWS) {
				...
				if (parseFloat(item.data.DISCOUNT_PRICE) > 0) {
			...
			if (this.options.showPriceWithoutDiscount && isDiscountPrice)
			{
						...
						children: [
							BX.create('A', {
								props: {
									...
									className: 'btn btn-primary btn-order-save'
								},
