;(function (window){
	...
	window.JCBlankZakazaItem = function (arParams)
	{
		this.DEBOUNCE_TIME = 500;
		this.arParams = arParams;
		this.itemId = arParams['PRODUCT']['ID'];
		this.node = document.getElementById(arParams['ITEM_IDS']['ID']);
		this.productType = arParams['PRODUCT']['CATALOG_TYPE'];
		this.showAbsent = true;
		this.showSkuProps = false;
		this.nodesQuantity = {
			wrapper: document.getElementById(arParams['ITEM_IDS']['QUANTITY']),
			increment: document.getElementById(arParams['ITEM_IDS']['QUANTITY_INCREMENT']),
			value: document.getElementById(arParams['ITEM_IDS']['QUANTITY_VALUE']),
			decrement: document.getElementById(arParams['ITEM_IDS']['QUANTITY_DECREMENT']),
		};
		this.nodePict = document.getElementById(arParams['ITEM_IDS']['PICTURE']);
		this.nodeAvaliable = this.node.querySelector('.product__property--avaliable');
		this.nodePrices = this.node.querySelectorAll('[data-entity="price-block"]');
		this.nodePriceMobile = this.node.querySelector('.product__property--price-mobile');
		this.nodeEmptyOffer = document.getElementById(arParams['ITEM_IDS']['OFFER_ROW_EMPTY']);
		this.nodeOffers = null;
		this.offers = {};
		this.treeProps = [];
		this.selectedValues = {};

		this.offersTogglerId = arParams['ITEM_IDS']['OFFERS_TOGGLER'];
		this.quantityTrace = arParams['PRODUCT'].CATALOG_QUANTITY_TRACE === "Y" ? true : false;
		this.canBuyZero = arParams['PRODUCT'].CATALOG_CAN_BUY_ZERO === "Y" ? true : false;
		this.ranges = [];
		this.prices = arParams['PRODUCT']['PRINT_PRICES'];
		this.delayAddToBasket = 0;

		this.obQuantity = {
			isSKU: false,
			itemId: arParams['PRODUCT']['ID'],
			name: arParams['PRODUCT']['NAME'],
			nodes: this.nodesQuantity,
			currentQuantity: parseFloat(arParams['PRODUCT']['ACTUAL_QUANTITY']) || 0,
			tmpQuantity: parseFloat(arParams['PRODUCT']['ACTUAL_QUANTITY']) || 0,
			maxQuantity: this.quantityTrace && !this.canBuyZero ? parseFloat(arParams['PRODUCT']['CATALOG_QUANTITY']) : Number.POSITIVE_INFINITY,
			minQuantity: 0,
			measureRatio: parseFloat(
				arParams['PRODUCT']['CATALOG_MEASURE_RATIO']
					? arParams['PRODUCT']['CATALOG_MEASURE_RATIO']
					: arParams['PRODUCT']['ITEM_MEASURE_RATIOS'].length > 0
						? arParams['PRODUCT']['ITEM_MEASURE_RATIOS'][arParams['PRODUCT']['ITEM_MEASURE_RATIO_SELECTED']]['RATIO']
						: 0
			),
			measureName: arParams['PRODUCT']['CATALOG_MEASURE_NAME']
				? arParams['PRODUCT']['CATALOG_MEASURE_NAME']
				: arParams['PRODUCT']['ITEM_MEASURE']['TITLE']
		};
		this.blockNodes = {};
		this.obTree = null;
			...
			case 3: // sku
				...
				if (arParams.OFFERS && BX.type.isArray(arParams.OFFERS) && arParams.OFFERS.length !== 0) {
					...
					if (arParams.OFFERS_VIEW !== 'BLOCK') {
						this.nodeOffers = document.getElementById(arParams['ITEM_IDS']['ID'] + '_offers');
					}
				}
	...
	window.JCBlankZakazaItem.prototype = {
		...
		initOffers: function() {
			if (this.arParams['PRODUCT']['OFFERS'].length === 0 && parseInt(this.productType) !== 3) {return}

			const offers = this.arParams['PRODUCT']['OFFERS'];
			offers.forEach(function (offer, index) {

				this.offers[index] = {
					id: offer.ID,
					name: offer.NAME ?? this.arParams.PRODUCT.NAME,
				...
				if (this.arParams.OFFERS_VIEW !== 'BLOCK') {
					const offerIds = this.arParams['ITEM_IDS']['OFFERS'][index];

					this.offers[index].node = document.getElementById(offerIds.ID);
					this.offers[index].obQuantity = this.getOfferQuantityPropsByIndex(index);
				}

			}.bind(this))

			if (this.productType === 3) {
				this.obTree = BX(this.arParams.ITEM_IDS.SKU_TREE);
				if (!this.obTree) {
					this.errorCode = -256;
				}
			}
		...
		getOfferQuantityPropsByIndex: function(index) {
			const offers = this.arParams['PRODUCT']['OFFERS']
			...
			if (window.innerWidth > 576) {
				this.nodePrices.forEach(item => {
					if (prices[item.dataset.code] && prices[item.dataset.code][this.offers[index].ITEM_QUANTITY_RANGE_SELECTED]) {
						...
						item.querySelector('span').innerHTML = price.PRINT;
		...
		setAvaliable: function (index) {
			...
			if (this.offers[index].measureRatio !== 1) {
				this.nodeAvaliable.querySelector('.product-ratio__title').innerText = BX.message('PRODUCT_LABEL_RATIO');
				this.nodeAvaliable.querySelector('.product-ratio__value').innerText = this.offers[index].measureRatio;
			} else {
				this.nodeAvaliable.querySelector('.product-ratio__title').innerText = '';
				this.nodeAvaliable.querySelector('.product-ratio__value').innerText = '';
			}

			if (this.arParams.SHOW_MAX_QUANTITY !== 'N' && window['ob_store_item_'+this.offers[index].id]?.setEventAvaliable) {
				this.nodeAvaliable.querySelector('.product__quant').classList.add('show-store');
				window['ob_store_item_'+this.offers[index].id].setEventAvaliable(this.nodeAvaliable.querySelector('.product__quant'));
			}
		},
		...
		redrawPrices: function (item) { //TODO: init price data once, not in every call of function
			...
			if (item.id === this.itemId) {
				if (this.arParams['PRODUCT']['ITEM_QUANTITY_RANGES'].hasOwnProperty('ZERO-INF')) {return}

				const ranges = this.arParams['PRODUCT']['ITEM_QUANTITY_RANGES'];
				let currentRange = this.arParams['PRODUCT']['ITEM_QUANTITY_RANGE_SELECTED'];
				...
				for (let price in this.arParams['ITEM_IDS']['PRICES']) {
					let node = document.getElementById(this.arParams['ITEM_IDS']['PRICES'][price]);
					if (node && price !== 'PRIVATE_PRICE') {
						let currentPrice = this.arParams['PRODUCT']['PRINT_PRICES'][price];
				...
				if (!this.arParams['ITEM_IDS']['OFFERS']?.hasOwnProperty(item.id)) {return}

				const offer = this.arParams['PRODUCT']['OFFERS'][item.id];
				...
				for (let price in this.arParams['ITEM_IDS']['OFFERS'][this.offerNum]['PRICES']) {
					let node = document.getElementById(this.arParams['ITEM_IDS']['OFFERS'][this.offerNum]['PRICES'][price]);

					if (node && price !== 'PRIVATE_PRICE') {
						let currentPrice = this.arParams['PRODUCT']['OFFERS'][item.id]['PRINT_PRICES'][price];
						if (currentPrice && currentPrice.hasOwnProperty(currentRange)) {
							node.innerHTML = currentPrice[currentRange]['PRINT'];
						} else {
							node.innerHTML = '';
