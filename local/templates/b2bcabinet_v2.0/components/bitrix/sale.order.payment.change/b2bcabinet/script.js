...
BX.Sale.OrderPaymentChange = (function()
{
	var classDescription = function(params)
	{
		...
		this.templateSigns = params.templateSigns;
	...
	classDescription.prototype.init = function()
	{
		...
		BX.bindDelegate(this.paySystemsContainer, 'click', { 'className': 'sale-order-payment-change-pp-company' }, BX.proxy(
			function(event)
			{
				var targetParentNode = event.target.closest('.sale-order-payment-change-pp-company');
				...
				BX.ajax(
					{
						...
						data:
						{
							...
							sotbit_set_site_template: this.templateSigns
						},
						onsuccess: BX.proxy(function(result)
						{
							const elementTitlePayment = this.wrapper.closest('.payment').querySelector('.payment-title')
							const titleTargetTypePayment = targetParentNode.querySelector('.sale-order-payment-change-pp-company-smalltitle').innerText;
							var btnChange = document.querySelector(".sale-order-detail-payment-options-methods-info-change-link");
							if(btnChange){
                                btnChange.style.display = "block";
							}

							elementTitlePayment.innerHTML = elementTitlePayment.innerHTML.replace(/(,)([^,]*$)/g, `, ${titleTargetTypePayment}`);
