...
$sort_by = $request['SORT']['CODE'] ?: $arParams['ELEMENT_SORT_FIELD'];
$sort_order = $request['SORT']['ORDER'] ?: $arParams['ELEMENT_SORT_ORDER'];
                        ...
                        <? foreach ($arParams['TABLE_HEADER'] as $propertyCode => $property) {
                            if (is_array($property) && $propertyCode == 'PRICES') {
                                foreach ($property as $singlePropertyCode => $singleProperty) {
                                    ...
                                    <th class="blank-zakaza__header-property
                                                <?=$arParams["ELEMENT_SORT_FIELD"] === ("PRICE_" . $singleProperty["ID"]) ? 'active' : ''?>"
                                        data-property-code="PRICE_<?=$singleProperty["ID"]?>"
                                        data-sort-order="<?= (!empty($sort_order) && $sort_by === "PRICE_" . $singleProperty["ID"]) ? $sort_order : '' ?>">
                                        <?=$singleProperty["NAME"]?>
                                    </th>
                                    <?
                                }
                            ...
                            elseif (is_array($property)) {
                                foreach ($property as $singlePropertyCode => $singleProperty) {
                                    ?>
                                    <th class="blank-zakaza__header-property
                                                <?=$arParams["ELEMENT_SORT_FIELD"] === $singlePropertyCode ? 'active' : ''?>"
                                ...
                                if ($property == $arParams['TABLE_HEADER']['NAME']) {
                                    ...
                                    <th colspan="2" class="blank-zakaza__header-property
                                        ...
                                        data-sort-order="<?= (!empty($sort_order) && $sort_by === $propertyCode) ? $sort_order : '' ?>">
                                    ...
                                    ?>
                                    <th class="blank-zakaza__header-property
                                                <?=$arParams["ELEMENT_SORT_FIELD"] === "PROPERTY_" . $propertyCode ? 'active' : ''?>"
                                        data-property-code="PROPERTY_<?=$propertyCode?>"
                                        data-sort-order="<?= (!empty($sort_order) && $sort_by === "PROPERTY_".$propertyCode) ? $sort_order : '' ?>">
                                        <?=$property?>
                                    </th>
                                    <?
                                }
...
<script>
    BX.message({
        ERROR_SORT: '<?=Loc::getMessage('ERROR_SORT')?>'
    });
