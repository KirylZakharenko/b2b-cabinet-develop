;(function(window){
    ...
    window.JCBlankZakaza.prototype = {
        init: function() {
            ...
            BX.addCustomEvent('SidePanel.Slider:onMessage', this.eventOnMessage.bind(this));
        },
                ...
                if (linkProduct = event.target.closest('.product__link')) {
                    ...
                    BX.SidePanel.Instance.open(
                        linkProduct.dataset.href,
                        {
                            width: 1344,
                            data: {
                                outIdItem: linkProduct.dataset.idItem
                            },
                            label: {
                                text: "",
                                color: "#9E9E9E",
        ...
        eventOnMessage: function (event) {
            ...
            if (window['ob'+event.data.obId] == null) {
                return;
            }

			if (event.data.id == window['ob'+event.data.obId].itemId) {
                item = window['ob'+event.data.obId].obQuantity;
			} else {
                item = window['ob'+event.data.obId].getOfferById(event.data.id).obQuantity
            }
			...
			item.nodes.value.value = event.data.quantity;
        ...
        processPagination: function (paginationHtml, data) {
            ...
            temporaryPagination.content.querySelectorAll('.page-link').forEach(link => {
                if (link.href) {
                    let urlPage = new URL(link.href, data.url.origin);
        ...
        changeSortHeader: function (node) {
            ...
            if (!this.prevNode) {
                this.wrapper.querySelectorAll('.blank-zakaza__header-property').forEach(item =>this.changeArrowForSort(item, node));
            } else {
                this.changeArrowForSort(this.prevNode, node);
            }
        ...
        changeArrowForSort: function (prevItem, currentItem) {
            if (prevItem !== currentItem) {
                prevItem.classList.remove('active');
                prevItem.dataset.sortOrder = '';
            }            
        }
    };
